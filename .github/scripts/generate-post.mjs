...
## 出力形式:
【重要】以下のJSON形式のみで返してください。余計な説明、コードブロック（\`\`\`）は一切含めないでください。JSONオブジェクトのみを出力してください。
...
async function updatePosts(newPost, repoDir) {
  ...
  // Step A: Write body Markdown to individual file
  const mdPath = path.join(postsDir, `${slug}.md`);
  await fs.writeFile(mdPath, newPost.body, 'utf-8');
...
async function main() {
  ...
  await updatePosts(blogPost, repoDir);
  clearTimeout(scriptTimer);
  console.log('\nDone!');
  process.exit(0);
}
